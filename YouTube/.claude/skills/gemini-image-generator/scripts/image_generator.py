...
def generate_image(prompt: str, output_path: str, show_browser: bool = False, timeout: int = 180):
    ...
    try:
        ...
        # Step 1: Find and click the "ツール" (Tools) button to open tool menu
        print("   → Looking for 'ツール' button...")
        tool_button_selectors = [
            'button:has-text("ツール")',
        ...
        tool_button = None
        for selector in tool_button_selectors:
            try:
                if page.locator(selector).count() > 0:
                    btn = page.locator(selector).first
                    if btn.is_visible():
                        tool_button = btn
                        print(f"   ✓ Found tool button: {selector}")
                        break
            except:
                continue

        if not tool_button:
            print("❌ Could not find 'ツール' button. UI may have changed.")
            print("   Try running with --show-browser to debug")
            context.close()
            playwright.stop()
            return False

        # Click tool button to open menu
        tool_button.click()
        page.wait_for_timeout(1500)

        # Step 2: Find and click "画像を作成" (Image generation) option
        print("   → Looking for '画像を作成' option...")
        image_gen_selectors = [
            '.mdc-list-item:has-text("画像を作成")',
            'button:has-text("画像を作成")',
            '[class*="menu"] button:has-text("画像を作成")',
            '[class*="drawer"] button:has-text("画像を作成")',
            '*:has-text("画像を作成"):visible',
        ]
        ...
        for selector in image_gen_selectors:
            try:
                ...
                if locator.count() > 0:
                    opt = locator.first
                    if opt.is_visible():
                        image_gen_option = opt
                        print(f"   ✓ Found image generation option: {selector}")
                        break
            except:
                continue

        if not image_gen_option:
            print("❌ Could not find '画像を作成' option in tool menu.")
            print("   Try running with --show-browser to debug")
            context.close()
            playwright.stop()
            return False

        # Click to open NanoBanana (image generation mode)
        image_gen_option.click()
        page.wait_for_timeout(3000)

        print("   → NanoBanana (画像を作成) activated")
